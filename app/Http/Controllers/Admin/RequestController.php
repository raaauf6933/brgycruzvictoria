...
class RequestController extends Controller
{
    public function index()
    {
        if(request()->ajax())
        {
            $requests = RequestResource::collection(Request::with('service', 'user.resident')->orderBy('status', 'DESC')->get());
                   ...
                   ->addColumn('actions', function($row) {
                    ...
                    $btn = "
                        <div class='dropdown'>
                            ...
                            <a class='dropdown-item' role='button' onclick='c_destroy($new_row[id],`resident.requests.destroy`,`.request_dt`)'>Delete</a>
    ...
    {
        return view('admin.request.show', ['request' => $request->load('service'), 'punong_barangay' => Official::whereRelation('position', 'name', 'Punong Barangay')->first()]);
    }
